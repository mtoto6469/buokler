...
//            }
            ...
            foreach ($main as $row) {
                ...
                if ($row->type != null && $row->type != "sllider" ) {
                ...
                }elseif ($row->type == "sllider"){
                    $slider = new Sliderapp();
                    $rowSlider = $slider->find()->where(["id_main"=>$row->id])->all();
                    if($rowSlider!=null){
                        $send['type'] = "slider";
                        foreach ($rowSlider as $rowS){
                            if($rowS->img !=null && $rowS->text !=null  && $rowS->id_m_d_f !=null ){

                                $sendS['text'] = $rowS->text;
                                $sendS['id_m_d_f'] = $rowS->id_m_d_f;
                                $sendS['img'] = $rowS->img;
                                $send['value'][] = $sendS ;
...
//در این قسمت جمع قیمت سبد خرید را محاسبه میکنیم
            ...
            $Scod =intval($_POST['codeOff']);
//            0 برای زمانی است که کاربر کدی ندارد وان را خالی وارد کرده است
//            در صورتی که غیر از ااین باشد ابتدا چک میشود ایا کد درست است یا خیر
//            در صورت درست نبودن به کاربر ارور فرستاده میشود
            if($Scod==0){
                $price=$price_all;
                $factor->code_off = 0;
            }else{
                //            کد ارسالی توسط کاربر چک میشود که ایا اعتبار دارد یا اصلا وجود دارد یا خیر
                $code=CodeOff::find()->where(['code'=>$Scod])->andWhere(['enabel_view'=>1])->one();
                if($code!=null){
//                در صورت صحت کد تخفیف این کد را باید غیر فعال کنیم به همین جهت enabel_view=0 میشود
                    $code->enabel_view=0;
                    $code->save();
                    $factor->code_off = $Scod;
//                قیمت نهایی با احتصاب کد تخفیف بدست میاید
                    $price=$price_all-($price_all*$code->price/100) ;
                }else{
                    $send['msg']= 'کد ارسالی درست نمیباشد یا قبلا استفاده شده است درصورت کد تخفیف نداید این فیلد را پر نکنید';
                    $send['result']=0;
                    return $send;
                }
            }
...
//                    در صورتی که تعداد برابر نبودند باید کدی که استفاده شده دوباره فعال شود
                $code=CodeOff::findOne(intval($_POST['codeOff']));
                if($code!=null){
                    $code->enabel_view=1;
                    $code->save();
                }
        ...
        if (isset($_POST['username']) && $_POST['username'] != null
            ...
            if ($user != null) {
                ...
                if ($user->validatePassword($password)) {
                    ...
                    print "نام کاربری یا پسورد اشتباه است";
...
//اینم درسته
//ولی باید چک کنید از لحاظ محاسباتی مشکلی نداشته باشد
...

    {
        if (isset($_POST['id_bag']) && $_POST['id_bag'] != null && isset($_POST['count']) && $_POST['count'] != null) {
            ...
            $bag2 = $tblBag->find()->where(['id' => $_POST['id_bag']])->andWhere(['id_user' =>Yii::$app->user->getId()])->one();
            if($bag2 == null){
                print "هیچ آیتمی پیدا نشد";
                exit;
            }
            ...
            $bag2->count = intval($_POST['count']);
            $bag2->price =$bag2->price  ."";
            if ($bag2->save()) {

                print "1";

            } else {
             var_dump($bag2->errors);
...
//بررسی شد
// این الان کار میکنه فقط سایز و رنگ و اینا را من رو صفر گذاشتم یه نگاه بهش بکن
...

    {
        ...
        if (isset($_POST['id']) && $_POST['id'] != null ) {
            ...
            $tblBag->id_fac = 0;
            $tblBag->enabel = 1;
            $tblBag->enable_view =1;
            $tblBag->down_buy = 0;
            $tblBag->size=-1;
            $tblBag->color="void";
            $tblBag->id_all_post = -1;

            $tblBag->price = $tblPro->price;
...
//اینم من دستکاری نکردم ولی کار میکنه
        ...
        if ($Bag != null) {
            ...
            foreach ($Bag as $rowBag) {
                ...
                if ($pro->exist !=0) {
                    ...
                    $createData["img"] = $url->hostInfo . "/upload/" . $img;
