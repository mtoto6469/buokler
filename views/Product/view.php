    ...
    <?= DetailView::widget([
        ...
        ['attribute'=>'id_category',
            ...
            ['attribute'=>'size',
                ...
                'value'=>function($model){
                    $size_pro='سایز های ';
                    $size=\backend\models\Tblsize::find()->where(['id_pro'=>$model->id])->all();
                    foreach ($size as $s){
            ...
            ['attribute'=>'price',
                'format'=>'html',
                'label'=>'قیمت برای کاربران به ترتیب ',
                'value'=>function($model){
                    $price_pro='قیمت  های ';
                    $price=\backend\models\Tblsize::find()->where(['id_pro'=>$model->id])->all();
                    foreach ($price as $s){
                        $price_pro.=$s->price .'<span style="color:red"> , </span>';
                    }
                    return $price_pro;
            ...
            ['attribute'=>'size',
                'format'=>'html',
                'label'=>'قیمت برای نماینده به ترتیب ',
                'value'=>function($model){
                    $price2_pro='قیمت ها ';
                    $price2=\backend\models\Tblsize::find()->where(['id_pro'=>$model->id])->all();
                    foreach ($price2 as $s){
                        $price2_pro.=$s->price_namayande .'<span style="color:red"> , </span>';
                    }
                    return $price2_pro;
